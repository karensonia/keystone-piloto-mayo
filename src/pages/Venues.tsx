...
const Venues = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6">
      <h1 className="text-3xl font-bold mb-8 text-gradient">Elige tu local</h1>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-4xl">
        {venues.map((venue) => (
          <Card key={venue.id} className="p-0 overflow-hidden shadow-lg group">
            <img src={venue.image} alt={venue.name} className="w-full h-48 object-cover group-hover:scale-105 transition-transform" />
            <div className="p-6 flex flex-col gap-3">
              <h2 className="font-bold text-xl text-foreground">{venue.name}</h2>
              <p className="text-muted-foreground text-sm">{venue.address}</p>
              <Button className="mt-2 w-full" onClick={() => handleEnter(venue)}>
                Entrar
              </Button>
            </div>
          </Card>
